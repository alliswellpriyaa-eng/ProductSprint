...
import { saveIdea, unsaveIdea, isIdeaSaved, type EtsyExportPack } from "@/lib/savedIdeas";
...
// EtsyExportPack is imported from @/lib/savedIdeas and re-used below

export interface PinterestPack {
  title: string;
  description: string;
  overlayText: string;
  hashtags: string[];
...
export interface ShortContentPack {
  hook: string;
  script: string;
  caption: string;
  hashtags: string[];
...
const etsyPackCache = new Map<string, EtsyExportPack>();
const pinterestCache = new Map<string, PinterestPack>();
const shortsCache = new Map<string, ShortContentPack>();
...
function EtsyPackModal({ idea, pack, onClose }: { idea: Idea; pack: EtsyExportPack; onClose: () => void }) {
  ...
  const CopyBtn = ({ text, id }: { text: string; id: string }) => (
    <button onClick={() => copyText(text, id)} className="text-xs text-gray-400 hover:text-purple-600 px-1 flex-shrink-0">
      {copied === id ? "✓" : "⎘"}
    </button>
  );
  ...
  const Section = ({ id, icon, title, children }: { id: string; icon: string; title: string; children: React.ReactNode }) => (
    <div className="border border-gray-100 rounded-xl overflow-hidden">
      ...
      >
        <span className="text-xs font-bold text-gray-700 flex items-center gap-2">{icon} {title}</span>
        <span className="text-gray-400 text-sm">{openSection === id ? "▲" : "▼"}</span>
      </button>
      {openSection === id && <div className="p-3 space-y-2">{children}</div>}
    </div>
  );

  const handleExportTxt = () => {
    const lines = [
      `ETSY PACK: ${idea.title}`,
      ...
      ``,
      `── LAUNCH CHECKLIST ──`,
      ...pack.launchChecklist.map((item, i) => `${i + 1}. ${item}`),
    ];
    const blob = new Blob([lines.join("\n")], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `etsy-pack-${idea.title.toLowerCase().replace(/\s+/g, "-").slice(0, 40)}.txt`;
    a.click();
    URL.revokeObjectURL(url);
    trackEvent("etsy_pack_exported", { idea: idea.title });
  };
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-start justify-between p-5 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl">
          <div>
            <p className="text-xs text-purple-600 font-semibold uppercase tracking-wide mb-1">📦 Etsy Export Pack</p>
        ...
        <div className="p-4 space-y-2">
          <Section id="seo" icon="🔍" title="SEO Title">
            <div className="flex items-start gap-2">
              <p className="flex-1 text-xs text-gray-800 bg-purple-50 rounded-lg p-2.5 leading-relaxed">{pack.seoTitle}</p>
              <CopyBtn text={pack.seoTitle} id="seoTitle" />
            </div>
          </Section>

          <Section id="tags" icon="#" title="Etsy Tags (13)">
            <div className="flex items-start gap-2">
              ...
              <CopyBtn text={pack.tags.join(", ")} id="tags" />
            </div>
          </Section>

          <Section id="desc" icon="📝" title="Listing Description">
            <div className="flex items-start gap-2">
              <p className="flex-1 text-xs text-gray-700 leading-relaxed whitespace-pre-line">{pack.description}</p>
              <CopyBtn text={pack.description} id="desc" />
            </div>
          </Section>

          <Section id="pricing" icon="💵" title="Pricing Suggestion">
            <p className="text-xs text-gray-700 bg-green-50 rounded-lg p-2.5">{pack.pricing}</p>
          </Section>

          <Section id="canva" icon="🎨" title="Canva Instructions">
            <p className="text-xs text-gray-700 leading-relaxed whitespace-pre-line bg-orange-50 rounded-lg p-2.5">{pack.canvaInstructions}</p>
          </Section>

          <Section id="thumb" icon="🖼" title="Thumbnail Text">
            <div className="flex items-start gap-2">
              <p className="flex-1 text-xs text-white font-bold bg-gray-900 rounded-lg p-2.5 uppercase tracking-wide">{pack.thumbnailText}</p>
              <CopyBtn text={pack.thumbnailText} id="thumb" />
            </div>
          </Section>

          <Section id="pinterest" icon="📌" title="Pinterest Content">
            <div className="space-y-2">
              <div className="flex items-start gap-2">
                <div className="flex-1">
                  <p className="text-xs font-semibold text-gray-500 mb-1">Pin Title</p>
                  <p className="text-xs text-gray-800 bg-red-50 rounded-lg p-2">{pack.pinterestTitle}</p>
                </div>
                <CopyBtn text={pack.pinterestTitle} id="pinTitle" />
              </div>
              <div className="flex items-start gap-2">
                <div className="flex-1">
                  <p className="text-xs font-semibold text-gray-500 mb-1">Pin Description</p>
                  <p className="text-xs text-gray-700 leading-relaxed">{pack.pinterestDescription}</p>
                </div>
                <CopyBtn text={pack.pinterestDescription} id="pinDesc" />
              </div>
            </div>
          </Section>

          <Section id="reel" icon="🎬" title="Reel / Shorts Caption">
            <div className="flex items-start gap-2">
              <p className="flex-1 text-xs text-gray-700 leading-relaxed whitespace-pre-line">{pack.reelCaption}</p>
              <CopyBtn text={pack.reelCaption} id="reel" />
            </div>
          </Section>

          <Section id="checklist" icon="✅" title="Launch Checklist">
            <ol className="space-y-1.5">
              {pack.launchChecklist.map((item, i) => (
                <li key={i} className="flex items-start gap-2 text-xs text-gray-700">
                  <span className="flex-shrink-0 w-4 h-4 rounded-full bg-purple-100 text-purple-700 text-xs font-bold flex items-center justify-center mt-0.5">{i + 1}</span>
                  {item}
                </li>
              ))}
            </ol>
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-purple-600 to-orange-500 hover:opacity-90 text-white font-bold py-3 rounded-xl transition-opacity text-sm mt-2"
          >
            ⬇️ Download as .txt
          </button>
        </div>
...
export default function IdeaCard({ idea, index, isPremium, platform, nicheData, onUpgradeClick, niche, productType }: IdeaCardProps) {
  ...
  // Export Etsy Pack
  const [etsyPack, setEtsyPack] = useState<EtsyExportPack | null>(() => etsyPackCache.get(idea.title) ?? null);
  const [loadingEtsyPack, setLoadingEtsyPack] = useState(false);
  const [showEtsyPackModal, setShowEtsyPackModal] = useState(false);
  const etsyPackFetchedRef = useRef(false);

  // Pinterest Helper
  const [pinterestPack, setPinterestPack] = useState<PinterestPack | null>(() => pinterestCache.get(idea.title) ?? null);
  const [loadingPinterest, setLoadingPinterest] = useState(false);
  const [showPinterestModal, setShowPinterestModal] = useState(false);
  const pinterestFetchedRef = useRef(false);

  // Shorts Helper
  const [shortsPack, setShortsPack] = useState<ShortContentPack | null>(() => shortsCache.get(idea.title) ?? null);
  ...
  // Initialise saved state from localStorage after mount
  useEffect(() => {
    setSaved(isIdeaSaved(idea.title));
  }, [idea.title]);
  ...
  const handleEtsyPack = useCallback(async () => {
    trackEvent("export_etsy_pack_clicked", { idea: idea.title });
    if (etsyPack) { setShowEtsyPackModal(true); return; }
    if (etsyPackFetchedRef.current) return;
    etsyPackFetchedRef.current = true;
    setLoadingEtsyPack(true);
    try {
      const res = await fetch("/api/generate-etsy-pack", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ idea: idea.title, niche, productType }),
      });
      const data = await res.json();
      if (res.status === 403 && data.upgradeRequired) { onUpgradeClick("export_pack_locked"); etsyPackFetchedRef.current = false; return; }
      if (!res.ok) throw new Error(data.error || "Failed");
      etsyPackCache.set(idea.title, data);
      setEtsyPack(data);
      setShowEtsyPackModal(true);
      trackEvent("etsy_pack_generated", { idea: idea.title });
    } catch { etsyPackFetchedRef.current = false; }
    finally { setLoadingEtsyPack(false); }
  }, [idea.title, niche, productType, onUpgradeClick, etsyPack]);

  const handlePinterest = useCallback(async () => {
    if (!isPremium) { trackEvent("premium_feature_clicked", { feature: "pinterest" }); onUpgradeClick("pinterest_helper"); return; }
    trackEvent("pinterest_helper_clicked", { idea: idea.title });
    if (pinterestPack) { setShowPinterestModal(true); return; }
    if (pinterestFetchedRef.current) return;
    pinterestFetchedRef.current = true;
    setLoadingPinterest(true);
    try {
      const res = await fetch("/api/generate-pinterest", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ idea: idea.title, niche }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed");
      pinterestCache.set(idea.title, data);
      setPinterestPack(data);
      setShowPinterestModal(true);
      trackEvent("pinterest_generated", { idea: idea.title });
    } catch { pinterestFetchedRef.current = false; }
    finally { setLoadingPinterest(false); }
  }, [idea.title, niche, isPremium, onUpgradeClick, pinterestPack]);

  const handleShorts = useCallback(async () => {
    if (!isPremium) { trackEvent("premium_feature_clicked", { feature: "shorts" }); onUpgradeClick("premium_feature_clicked"); return; }
    trackEvent("shorts_helper_clicked", { idea: idea.title });
    if (shortsPack) { setShowShortsModal(true); return; }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed");
      shortsCache.set(idea.title, data);
      setShortsPack(data);
      setShowShortsModal(true);
      trackEvent("shorts_generated", { idea: idea.title });
  ...
  const toggleSave = useCallback(() => {
    trackEvent("save_idea_clicked", { idea: idea.title });
    ...
    if (saved) {
      ...
      saveIdea({
        ...
        exportPack: etsyPack ?? undefined,
        savedAt: new Date().toISOString(),
      });
      setSaved(true);
      trackEvent("idea_saved", { idea: idea.title, niche });
    }
  }, [idea, niche, productType, saved, isPremium, onUpgradeClick, etsyPack]);
  ...
  return (
    <>
      ...
      >
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            {/* Export Etsy Pack — free for all users */}
            <button
              data-testid="etsy-pack-button"
              onClick={handleEtsyPack}
              disabled={loadingEtsyPack}
              className="col-span-2 text-xs py-2 px-2 rounded-lg font-medium transition-colors flex items-center justify-center gap-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {loadingEtsyPack ? <><Spinner size={12} className="text-emerald-600" /> Building pack…</> : "📦 Export Etsy Pack"}
            </button>

            {/* Pinterest Helper — premium */}
            <button
              onClick={handlePinterest}
              disabled={loadingPinterest}
              className="text-xs py-2 px-2 rounded-lg font-medium transition-colors flex items-center justify-center gap-1 bg-red-50 text-red-600 hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {loadingPinterest ? <Spinner size={12} className="text-red-500" /> : "📌 Pinterest"} {!isPremium && <span className="opacity-60">🔒</span>}
            </button>

            {/* Shorts Helper — premium */}
            <button
              onClick={handleShorts}
              disabled={loadingShorts}
              className="text-xs py-2 px-2 rounded-lg font-medium transition-colors flex items-center justify-center gap-1 bg-violet-50 text-violet-700 hover:bg-violet-100 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {loadingShorts ? <Spinner size={12} className="text-violet-600" /> : "🎬 Shorts"} {!isPremium && <span className="opacity-60">🔒</span>}
            </button>
      ...
      {showEtsyPackModal && etsyPack && <EtsyPackModal idea={idea} pack={etsyPack} onClose={() => setShowEtsyPackModal(false)} />}

      {/* Pinterest Quick Modal */}
      {showPinterestModal && pinterestPack && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && setShowPinterestModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[85vh] overflow-y-auto">
            <div className="flex items-start justify-between p-5 border-b border-gray-100">
              <div>
                <p className="text-xs text-red-500 font-semibold uppercase tracking-wide mb-1">📌 Pinterest Helper</p>
                <h3 className="text-base font-bold text-gray-900">{idea.title}</h3>
              </div>
              <button onClick={() => setShowPinterestModal(false)} className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-500">✕</button>
            </div>
            <div className="p-4 space-y-3 text-xs text-gray-700">
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Pin Title</p><p className="bg-red-50 rounded-lg p-2.5">{pinterestPack.title}</p></div>
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Description</p><p className="leading-relaxed">{pinterestPack.description}</p></div>
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Overlay Text</p><p className="bg-gray-900 text-white font-bold rounded-lg p-2.5 uppercase tracking-wide">{pinterestPack.overlayText}</p></div>
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Hashtags</p><div className="flex flex-wrap gap-1.5">{pinterestPack.hashtags.map((h, i) => <span key={i} className="bg-red-50 text-red-600 rounded-full px-2.5 py-0.5">{h}</span>)}</div></div>
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Call to Action</p><p className="text-purple-700 font-medium">{pinterestPack.cta}</p></div>
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Canva Layout</p><p className="text-gray-600 leading-relaxed">{pinterestPack.canvaLayout}</p></div>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Shorts Quick Modal */}
      {showShortsModal && shortsPack && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && setShowShortsModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[85vh] overflow-y-auto">
            <div className="flex items-start justify-between p-5 border-b border-gray-100">
              <div>
                <p className="text-xs text-violet-600 font-semibold uppercase tracking-wide mb-1">🎬 Shorts / Reels Helper</p>
                <h3 className="text-base font-bold text-gray-900">{idea.title}</h3>
              </div>
              <button onClick={() => setShowShortsModal(false)} className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-500">✕</button>
            </div>
            <div className="p-4 space-y-3 text-xs text-gray-700">
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Hook</p><p className="bg-violet-50 text-violet-800 font-semibold rounded-lg p-2.5 leading-relaxed">{shortsPack.hook}</p></div>
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Script</p><p className="leading-relaxed whitespace-pre-line">{shortsPack.script}</p></div>
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Caption</p><p className="leading-relaxed">{shortsPack.caption}</p></div>
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Hashtags</p><div className="flex flex-wrap gap-1.5">{shortsPack.hashtags.map((h, i) => <span key={i} className="bg-violet-50 text-violet-700 rounded-full px-2.5 py-0.5">{h}</span>)}</div></div>
              <div><p className="font-bold text-gray-500 uppercase tracking-wider mb-1">Call to Action</p><p className="text-purple-700 font-medium">{shortsPack.cta}</p></div>
            </div>
          </div>
        </div>
      )}
